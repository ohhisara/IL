...
happyExpList = Happy_Data_Array.listArray (0,78) ([33232,1030,0,0,1920,128,0,32768,0,26653,33232,7430,104,0,28573,30720,8193,0,4,26653,33232,7430,53352,1665,0,0,0,0,0,26653,33232,6,53256,1785,0,33232,6,8192,120,26653,30720,1,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_syntax","Term","let","in","int","var","'\\\\'","'.'","'='","'+'","'-'","'*'","'/'","'('","')'","ifzero","fix","%eof"]
        bit_start = st * 20
        bit_end = (st + 1) * 20
...
action_21 (12) = happyShift action_15
...
action_30 (6) = happyShift action_31
...
happyReduction_9 ((HappyAbsSyn4  happy_var_4) `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn4
		 (Ifzero happy_var_2 happy_var_3 happy_var_4
	) `HappyStk` happyRest
...
syntax tks = happyRunIdentity happySomeParser where
